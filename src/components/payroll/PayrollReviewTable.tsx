      ...
      <div className="rounded-lg border bg-muted/40 p-4 space-y-4">
        ...
        <div className="flex flex-wrap items-end gap-4 border-t pt-3">
          <div>
            ...
            <NumInput value={global.taxRate} onChange={v => setGlobal(g => ({ ...g, taxRate: v }))} className="w-20" />
          </div>
          <div>
            <p className="text-xs font-medium text-muted-foreground mb-1.5">Fee per payslip (£)</p>
            <NumInput value={global.feeAmount} onChange={v => setGlobal(g => ({ ...g, feeAmount: v }))} min={0} max={99999} step={1} prefix="£" suffix="" className="w-24" />
          </div>
          <div className="border-l pl-4">
            <p className="text-xs font-medium text-muted-foreground mb-1.5">Fee split — Umbrella %</p>
            <NumInput value={global.umbrellaSharePct} onChange={v => setGlobal(g => ({ ...g, umbrellaSharePct: v, brokerSharePct: parseFloat((100 - v).toFixed(1)) }))} className="w-20" />
          </div>
          <div>
            <p className="text-xs font-medium text-muted-foreground mb-1.5">Broker %</p>
            <NumInput value={global.brokerSharePct} onChange={v => setGlobal(g => ({ ...g, brokerSharePct: v, umbrellaSharePct: parseFloat((100 - v).toFixed(1)) }))} className="w-20" />
          </div>
          <div className="border-l pl-4">
              ...
              return (
                <tr key={e.id} className="hover:bg-muted/30 transition-colors">
                  ...
                  <td className="px-3 py-2.5 text-center">
                    <NumInput value={er.taxRate} onChange={v => setRate(e.id, 'taxRate', v)} disabled={isPending} className="w-16" />
                  </td>
                  <td className="px-3 py-2.5 text-right tabular-nums text-red-600">-{fmt(c.tax)}</td>
                  <td className="px-3 py-2.5 text-center">
                    <NumInput value={er.feeAmount} onChange={v => setRate(e.id, 'feeAmount', v)} min={0} max={99999} step={1} prefix="£" suffix="" disabled={isPending} className="w-20" />
                  </td>
                  <td className="px-3 py-2.5 text-center">
                    <NumInput value={er.umbrellaSharePct}
                      ...
                      disabled={isPending} className="w-16" />
                  </td>
                  ...
                  <td className="px-3 py-2.5 text-center">
                    <NumInput value={er.expenseAmount} onChange={v => setRate(e.id, 'expenseAmount', v)} min={0} max={99999} step={1} prefix="£" suffix="" disabled={isPending} className="w-20" />
                  </td>
